...
TOC_CHUNK_PATTERN = re.compile(r"目\s*录")


class MarkdownChunkOperator(Operator):
    """Chunk the markdown file into chunks of text, skip the chunks that contain "目录"."""
    ...
    def run(self, rows: list[Record]) -> list[Record]:
        ...
        for row in rows:
            ...
            for idx, start in enumerate(range(0, len(words), step)):
                ...
                if TOC_CHUNK_PATTERN.search(chunk_text) or "Contents" in chunk_text or "Conent" in chunk_text:
                    continue
                chunks.append(
                    {
