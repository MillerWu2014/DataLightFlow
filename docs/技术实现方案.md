...
| `pipeline_depth_qa` | `run_depth_qa_pipeline` | `AgenticQAPipelineResult` | `{input_path.parent}/qa_depth.jsonl` |
| `pipeline_width_qa` | `run_width_qa_pipeline` | `AgenticQAPipelineResult` | `{input_path.parent}/qa_width.jsonl` |
...
- `pipeline_depth_qa` / `pipeline_width_qa` 的默认输出与 **`input_path` 同目录**（`qa_depth.jsonl` / `qa_width.jsonl`）；扩写 / think 则回退到配置 `output.root` 下子路径。
...
|------|------|------|
| `n_rounds` | `2` | 深挖轮数 |
| `output_path` | `{input_path.parent}/qa_depth.jsonl` | 输出 JSONL；省略时与输入文件同目录 |
...
```python
# 省略 output_path → 写入 input 同目录 qa_depth.jsonl
depth = service.pipeline_depth_qa(
    input_path=Path("outputs/singlehop/doc/taxonomy/qa_generated.jsonl"),
    n_rounds=2,
    lmstudio=True,
)
# depth.output_path == Path("outputs/singlehop/doc/taxonomy/qa_depth.jsonl")
...
| 参数 | 默认 | 说明 |
|------|------|------|
| `output_path` | `{input_path.parent}/qa_width.jsonl` | 输出 JSONL；省略时与输入文件同目录 |
...
```python
# 省略 output_path → 写入 input 同目录 qa_width.jsonl
width = service.pipeline_width_qa(
    input_path=Path("outputs/singlehop/doc/taxonomy/qa_generated.jsonl"),
    lmstudio=True,
)
```
...
pipeline_markdown_qa → qa_generated.jsonl
  ├→ pipeline_depth_qa  → qa_depth.jsonl   （默认与 input 同目录）
  └→ pipeline_width_qa  → qa_width.jsonl   （默认与 input 同目录；输入需 ≥2 行）
```
